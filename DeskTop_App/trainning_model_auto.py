...
def train_predict_from_img(path_img):
    tic = time.perf_counter()
    print("Training KNN classifier...")
    # Creates Classifier
    classifier = train(TRAIN_DIR, model_save_path="trained_knn_model.clf", verbose=verbose)
    ...
    if verbose:
        print(f"Time to train {toc - tic:0.4f} seconds")
    predictions = predict(path_img, model_path="trained_knn_model.clf")
    for name, (top, right, bottom, left) in predictions:
        print("- Found {} at ({}, {})".format(name, left, top))
    show_prediction_labels_on_image(path_img, predictions)

def predict_from_image(path_img):
...
def predict_frame(frame,knn_clf=None, model_path=None, distance_threshold=0.5):
    if knn_clf is None and model_path is None:
        raise Exception("Must supply knn classifier either thourgh knn_clf or model_path")
    ...
    if knn_clf is None:
        with open(model_path, 'rb') as f:
            knn_clf = pickle.load(f)
    X_face_locations = rec_face.detect_face(frame)
...
def predict_from_camera():
    ...
    while True:
        ...
        predictions = predict_frame(frame,model_path="trained_knn_model.clf")
        for name, (top, right, bottom, left) in predictions:
            print("- Found {} at ({}, {})".format(name, left, top))
        for name, (top, right, bottom, left) in predictions:
            top_right = (right, top)
...
if __name__ == '__main__':
    predict_from_camera()
    # train(train_dir="dataset",model_save_path="model_test.clf")